...
class DataCentreVideo extends StatefulWidget {
  ...
  final from;
  @override
  _DataCentreVideoState createState() => _DataCentreVideoState();
}

class _DataCentreVideoState extends State<DataCentreVideo> {
  ...
  Future<void> loadText() async {
    final response = await http.get(Uri.https(MOKIO_BASE_URL, DATACENTRE_TEXT));
    dataCentreTexts = json.decode(response.body);
  }

  videoHandler() async {
    _controller = VideoPlayerController.network(datacentre_REV);
    ...
    _motorVideoController = VideoPlayerController.network(datacentre_MOTOR)
      ..initialize().then((_) => {
            setState(() {
    ...
    _energySavingVideoController =
        VideoPlayerController.network(datacentre_MOTOR)
          ..initialize().then((_) => {
                setState(() {
    ...
    _mapVideoController = VideoPlayerController.network(datacentre_MAP)
      ..initialize().then((_) => {
            setState(() {
                              ...
                              show
                                  ? Positioned(
                                      left: Utils.getVideoScreenWidth(
                                              screenSizeMobile1) *
                                          0.5,
                                      top: Utils.getVideoScreenHeight(
                                              screenSizeMobile1) *
                                          0.55,
                                      child: Container(
                                        width: Utils.getVideoScreenWidth(
                  ...
                  show
                      ? Positioned(
                          ...
                          child: Container(
                            ...
                            height:
                                Utils.getVideoScreenHeight(screenSize) * 0.2,
                            decoration: const BoxDecoration(
                              image: DecorationImage(
                                image: AssetImage(dataCenterAirFlowGif),
                                fit: BoxFit.fill,
                              ),
                            ),
                          ),
