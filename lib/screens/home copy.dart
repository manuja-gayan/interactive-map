...
class HomeCopy extends StatefulWidget {
  const HomeCopy({Key? key, this.offsetHor, this.offsetVer}) : super(key: key);
  final offsetHor;
  final offsetVer;
  @override
  _HomeCopyState createState() => _HomeCopyState();
}

class _HomeCopyState extends State<HomeCopy> {
  late Timer _timer;
  void startTimer() {
    _timer =
        Timer.periodic(const Duration(milliseconds: 100), (Timer timer) async {
      //print(await _buildingsVideoController.position);
      final bool isPlaying = _buildingsVideoController.value.isPlaying;
      print(isPlaying);
      Duration? d = await _buildingsVideoController.position;
      if (_buildingsVideoController.value.duration.inMilliseconds -
              d!.inMilliseconds <=
          120) {
        // setState(() {
        ...
        _buildingsVideoController.pause();
        ...
        if (index > 1) {
          // setState(() {
          //   _buildingsVideoPlaying = false;
          // });
          //await Future.delayed(
          //    const Duration(milliseconds: 1500));
          // _buildingsVideoController
          //     .seekTo(_buildingsVideoController.value.duration);
            ...
            Center(
              child: InkWell(
                onTap: () async {
                  ...
                  setState(() {
                    endStillImage =
                        'assets/tempory images/Buildings_menu_still.png';
                    showEndStill = true;
                  });

                  _buildingsVideoController.addListener(() async {
